...
b1 = signal.firwin(N_TAP, cutoff, window=('kaiser', beta), fs=FS * OVERSAMPLING_RATIO)

# 最小位相FIRに変換（librosaのhilbertベース）
b1_minimum = signal.minimum_phase(b1, method='hilbert')

fir = FIR_filter(b1_minimum)
...
print('CMSIS')
print_filter_params(np.flip(b1_minimum), 'fir')
print('\n')
print('general use')
print_filter_params(b1_minimum, 'fir')
